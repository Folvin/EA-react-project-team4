					...
					>
						<Listbox.Options
							...
							{arr.map((nation, nationIdx) => (
								<Listbox.Option
									key={nationIdx}
									className={({ active }) => `relative cursor-default select-none py-1 pl-4 pr-4 ${active ? 'bg-blue-700 text-white' : 'text-gray-900'}`}
									value={nation}
								>
									{({ selected }) => (
										<>
											...
											>
												<span>
													<img src={nation.img} alt='' className='w-[12px] float-left' />
													{nation.name}
												</span>
											</span>
											{selected ? (
